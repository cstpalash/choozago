          ...
          if(parsedInput.command){
            switch(parsedInput.command.type){
              ...
              case "show":
                ...
                break;
              case "cancel":
                return ticket.cancel(user, parsedInput.command.data.ticketId);
                break;
              case "exit":
	...
	if(parsedInput.isEmail && parsedInput.isValidDomain){
			return registration.addUserDetails(parsedInput.userid, 
				...
				parsedInput.time).then(function(addedUser){
					...
					return "Please check your email for verification code and type below. If not received please check spam/junk folder."
				});
	}
